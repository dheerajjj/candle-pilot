...
The paper quantity is capped at ten shares and by the configured ₹5,000 per-order and daily-buy limits, using the current quote and paper cash. It represents a simulation, not a recommended allocation for your real portfolio. The app currently scans your NSE holdings plus five built-in watchlist names, at most ten stocks total; it does not search every listed company or guarantee returns. This extra context applies to desktop **paper** decisions only; do not turn on experimental live execution based on it. The feed is public and may be delayed, incomplete, or rate-limited. [Kite market data reference](https://kite.trade/docs/connect/v3/market-quotes/); [GDELT DOC API reference](https://blog.gdeltproject.org/gdelt-doc-2-0-api-debuts/).
...
3. On each trading morning, double-click `CandlePilot.cmd` and press **Connect Kite and see suggestions**. Complete the official Zerodha browser login; the local callback finishes without copying tokens. The read-only run then scans your current NSE holdings and five default watchlist stocks (INFY, RELIANCE, TCS, HDFCBANK, ICICIBANK), reads available funds, and shows its suggestions. Instrument tokens are resolved from Kite automatically. Run between **09:20 and 14:55 IST** on trading weekdays.
