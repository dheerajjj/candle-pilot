...
The desktop screen now **reads your Kite equity funds** and displays Kite's current available balance, opening balance, raw cash, utilised debits and collateral, plus the current value and P&L reported for NSE holdings. Position sizing uses `available.live_balance`, which Zerodha defines as the current available balance; it does not incorrectly reduce the balance to zero when another funds field is zero. The recommendation budget is the lesser of half the available balance or ₹5,000; each name is limited to ₹2,500 and ten shares. For example, with ₹10,000 available it may suggest two qualifying stocks with quantities fitting within a total ₹5,000 budget, keeping at least ₹5,000 untouched. If no stock clears all checks, it suggests no buy. Existing Kite holdings do not get duplicate buy suggestions. An estimate is not a reserved balance, and prices can change before an order is placed.
